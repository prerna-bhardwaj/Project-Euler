...
class LongestProduct {
	...
	static long get_down(int a, int b)
	{
	    if(a > 16)  return -1;
	    return arr[a][b] * arr[a+1][b] * arr[a+2][b] * arr[a+3][b]; 
	}

	static long get_right(int a, int b)
	{
	    if(b > 16)  return -1;
	...
	static long get_diag_for(int a, int b)
	{
	    if(a > 16 || b > 16)  return -1;
	    return arr[a][b] * arr[a+1][b+1] * arr[a+2][b+2] * arr[a+3][b+3]; 
	}


	static long get_diag_back(int a, int b)
	{
	    if(a > 16 || b < 3)  return -1;
	    return arr[a][b] * arr[a+1][b-1] * arr[a+2][b-2] * arr[a+3][b-3]; 
	}

	static long get_product(int a, int b)
	{
	    long ans = 0;   
	    if(get_right(a,b) > ans)   ans = get_right(a,b);
	    if(get_down(a,b) > ans)   ans = get_down(a,b);
	    if(get_diag_for(a,b) > ans)   ans = get_diag_for(a,b);
	    if(get_diag_back(a,b) > ans)   ans = get_diag_back(a,b);
